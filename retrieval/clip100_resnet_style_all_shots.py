...
# 使用CLIP进行第一阶段检索
def clip_first_stage_retrieval(query_feature, coco_features, coco_paths, inpainted_features=None, inpainted_paths=None, top_k=100):
    """第一阶段：使用CLIP特征检索top_k个结果
    
    如果提供了inpainted_features和inpainted_paths，将同时从COCO和inpainted图像中检索
    """
    # 如果同时有COCO和inpainted特征，则合并它们
    if inpainted_features is not None and len(inpainted_features) > 0 and inpainted_paths is not None and len(inpainted_paths) > 0:
        print(f"将从COCO数据集({len(coco_features)}张图像)和inpainted图像({len(inpainted_features)}张图像)中联合检索")
        # 合并特征和路径
        all_features = np.vstack([coco_features, inpainted_features])
        all_paths = coco_paths + inpainted_paths
    else:
        print(f"仅从COCO数据集({len(coco_features)}张图像)中检索")
        all_features = coco_features
        all_paths = coco_paths
...
# 按类别分组的检索函数
def retrieve_by_category(dataset_name, shot_count, clip_model, clip_preprocess, resnet_model, coco_features, coco_paths, device, force_recompute_inpainted=False):
    """为数据集中的每个类别进行检索"""
    # 获取inpainted图像和类别信息
    ...
    for category_name, samples in tqdm(category_to_samples.items(), desc=f"处理 {dataset_name} ({shot_count}_shot) 的类别"):
        ...
        for sample_id in samples:
            ...
            # 使用图像特征作为查询
            # 第一阶段：CLIP检索（同时从COCO和inpainted图像中检索）
...
def main():
    ...
    for dataset_name in args.datasets:
        ...
        for shot_count in args.shots:
            ...
            # 按类别进行检索
            dataset_results = retrieve_by_category(
                dataset_name, 
                shot_count,
                clip_model, 
                clip_preprocess, 
    ...
    if any(all_shots_results.values()):
        ...
        print("=============================================================")
        print("图像检索完成！结果包括:")
        print(f"1. COCO图像 + inpainted图像的联合检索")
